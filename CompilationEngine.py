...
class CompilationEngine:
    ...
    def compileParameterList(self):
        ...
        self.tk.advance()
        if self.tk.symbol() != ')':
            self.process(['keyword', 'identifier'], ['int', 'char', 'boolean'], False)
            self.process('identifier')
            self.tk.advance()
            while not (self.tk.tokenType() == 'symbol' and self.tk.symbol() == ')'):
                self.process('symbol', ',', False)
    ...
    def compileVarDec(self):
        ...
        self.process('keyword', 'var', False)
        self.process(['keyword', 'identifier'], ['int', 'char', 'boolean'])
        while True:
            self.process('identifier')
            self.process('symbol', [',', ';'])
    ...
    def compileReturn(self):
        ...
        self.process('keyword', 'return', False)
        self.tk.advance()
    ...
    def process(self, expectedTypes, expectedTokens='', advance=True, xmlOut = True):
        if advance:
            self.tk.advance()
        ...
        if xmlOut:
            if self.tk.symbol() in self.XMLSymDict:
                self.outputString += self.indent + '<'+self.tk.tokenType()+'>'+self.XMLSymDict[token]+'</'+self.tk.tokenType()+'>\n'
            else:
                self.outputString += self.indent + '<'+self.tk.tokenType()+'>'+token+'</'+self.tk.tokenType()+'>\n'
